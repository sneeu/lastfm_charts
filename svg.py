...
import os
display_prog = 'open -a /Applications/Safari.app/' # Command to execute to display images.
...
class Line:
    ...
    def strarray(self):
        ...
        return ["  <line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\"%s />\n" %\
                (self.start[0],self.start[1],self.end[0],self.end[1], style)]
...
class Text:
    def __init__(self,origin,text,style=None):
        self.origin = origin
        self.text = text
        self.style = style
        return
    ...
    def strarray(self):
        ...
        return ["  <text x=\"%d\" y=\"%d\" %s>\n" %\
                (self.origin[0],self.origin[1], style),
                "   %s\n" % self.text,
                "  </text>\n"]
